...
const CAMPOS_CERT = [
    ...
    { key: "representante_legal", label: "Representante legal (nombre)", required: false },
    { key: "representante_legal_puesto", label: "Cargo del representante legal", required: false },
...
async function handleFile(file) {
    ...
    try {
        ...
        if (data.error) {
            ...
            dropzone.querySelector(".dropzone-text").innerHTML = "Arrastra tu Excel aquí<br>o <strong>haz click para seleccionar</strong>";
            dropzone.querySelector(".dropzone-icon").textContent = "📄";
            return;
        }
...
function validarCURP(curp) {
    ...
    if (c.length !== 18) return { ok: false, msg: `${c.length} chars (deben ser 18)` };
...
function renderTemplateCategories() {
 ...
 card.dataset.template = group.templates.length === 1 ? group.templates[0] : "";
 card.innerHTML = `
...
function showSubTemplates(group) {
    ...
    document.getElementById("subCategoryTitle").textContent = group.icon + " " + group.name;
...
function selectTemplate(name) {
    ...
    if (card) {
        ...
        if (!card.querySelector(".sel-check")) {
            ...
            check.textContent = "✅";
            check.style.cssText = "position:absolute;top:-6px;right:-6px;font-size:20px;z-index:5";
...
function construirMapeo() {
    ...
    const camposAMostrar = state.camposDisponibles.length > 0
    ...
    camposAMostrar.forEach(campo => {
        ...
        const matchIdx = headers.findIndex(h => h && normalize(h).includes(campoNorm));
        if (matchIdx >= 0) {
...
function actualizarResumen() {
    ...
    let curpOk = 0, curpBad = 0, rfcOk = 0, rfcBad = 0;
    let errores = [];

    state.excelData.rows.forEach((row, idx) => {
        let nombre = nombreCol >= 0 ? (row[nombreCol] || `Fila ${idx+1}`) : `Fila ${idx+1}`;
        let rowErrors = [];

        if (curpCol >= 0) {
            const v = validarCURP(row[curpCol] || "");
            if (v.ok) curpOk++; else { curpBad++; rowErrors.push(`CURP: ${v.msg}`); }
        }
        if (rfcCol >= 0) {
            const v = validarRFC(row[rfcCol] || "");
            if (v.ok) rfcOk++;
            else { rfcBad++; rowErrors.push(`RFC: ${v.msg}`); }
        }
    ...
    if (curpCol >= 0 || rfcCol >= 0) {
        ...
        if (hasErrors) {
            ...
            const erroresTexto = errores.map(e =>
                `${e.nombre}: ${e.errors.join(", ")}`
            ).join("\n");
    ...
    if (state.selectedTemplate === "dc3") {
        ...
        <div class="summary-section" style="margin-top:8px;padding-top:8px;border-top:1px solid #e2e8f0">
            <div class="summary-row">
                <span class="summary-label">👤 Representante legal</span>
                <span class="summary-value">
                    <input type="text" id="inputRepLegal" placeholder="Ing. Alejandro García Salinas"
                           style="border:1px solid #cbd5e1;border-radius:6px;padding:4px 8px;font-size:12px;width:200px"
                           value="Ing. Alejandro García Salinas">
                </span>
            </div>
            <div class="summary-row">
                <span class="summary-label">🏢 Cargo</span>
                <span class="summary-value">
                    <input type="text" id="inputRepCargo" placeholder="Director General · AGASI"
                           style="border:1px solid #cbd5e1;border-radius:6px;padding:4px 8px;font-size:12px;width:200px"
                           value="Director General · AGASI">
                </span>
            </div>
            <div class="summary-row">
                <span class="summary-label">✍️ Firma del representante legal</span>
                <span class="summary-value">
...
document.getElementById("btnPreview").addEventListener("click", async () => {
    ...
    const requiredFields = CAMPOS_CERT.filter(c => c.required && state.camposDisponibles.includes(c.key));
...
document.getElementById("btnGenerate").addEventListener("click", async () => {
    ...
    const filas = state.excelData.rows.map(row => {
        ...
        if (state.selectedTemplate === "dc3") {
            ...
            const repCargo = document.getElementById("inputRepCargo");
            const chkRep = document.getElementById("chkRepTrabajadores");
            datos["representante_legal"] = repLegal ? repLegal.value : "Ing. Alejandro García Salinas";
            datos["representante_legal_puesto"] = repCargo ? repCargo.value : "Director General · AGASI";
            datos["firma_representante"] = firmaLegalData || "firmas/Firma_Soledad_Pastorutti.png";
            datos["representante_trabajadores"] = chkRep && chkRep.checked
                ? (document.getElementById("inputRepTrabajadores")?.value || "Representante")
                : "";
            datos["firma_trabajadores"] = chkRep && chkRep.checked ? firmaTrabData : "";
        }
        return datos;
    });
...
function iniciarProgreso() {
    ...
    evtSource.onmessage = (e) => {
        ...
        current.textContent = data.current_name ? `Procesando: ${data.current_name}` : "";
